<?php defined( 'ABSPATH' ) or die( 'No script kiddies please!' ); ?>

<div class="col-10 mt-5 simulator-footer">
  <div class="btn-group mr-4">
    <button type="button" class="btn btn-secondary mr-2"><i class="fa fa-expand"></i></button>
    <button type="button" class="btn btn-secondary ml-2"><i class="fa fa-list"></i></button>
  </div>
  <div class="btn-group mr-4">
    <button type="button" class="btn btn-secondary mr-2"><i class="fa fa-font"></i></button>
    <button type="button" class="btn btn-secondary ml-2"><i class="fa fa-lg fa-font"></i></button>
  </div>
  <div class="float-right">
    <button type="button" class="btn btn-primary">
      <?php echo __('New Simulator', 'enem-simulator') ?> 
      <i class="fa fa-undo"></i>
    </button>
  </div>
</div>
